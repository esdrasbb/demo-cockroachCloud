...
def create_accounts(conn):
    with conn.cursor() as cur:
        ...
        logging.info("create_accounts(): status message: %s", cur.statusmessage)
    conn.commit()
...
def delete_accounts(conn):
    with conn.cursor() as cur:
        ...
        logging.info("delete_accounts(): status message: %s", cur.statusmessage)
    conn.commit()
...
def print_balances(conn):
    with conn.cursor() as cur:
        ...
        logging.info("print_balances(): status message: %s", cur.statusmessage)
        rows = cur.fetchall()
        conn.commit()
...
def transfer_funds(conn, frm, to, amount):
    ...
    conn.commit()
    logging.info("transfer_funds(): status message: %s", cur.statusmessage)
    ...
    with conn:
        for retry in range(1, max_retries + 1):
            try:
                ...
                conn.rollback()
                logging.error("EXECUTE SERIALIZATION_FAILURE BRANCH")
                sleep_ms = (2 ** retry) * 0.1 * (random.random() + 0.5)
                logging.error("Sleeping %s seconds", sleep_ms)
                time.sleep(sleep_ms)

            except psycopg2.Error as e:
                logging.error("got error: %s", e)
                logging.error("EXECUTE NON-SERIALIZATION_FAILURE BRANCH")
                raise e
...
def test_retry_loop(conn):
    ...
    logging.info("test_retry_loop(): status message: %s", cur.statusmessage)
...
def main():
    ...
    try:
        ...
        logging.error("run_transaction(conn, op) failed: %s", ve)
        pass
